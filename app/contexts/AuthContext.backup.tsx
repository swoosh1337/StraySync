  ...
  const fetchProfile = async (userId: string): Promise<void> => {
    try {
      ...
      try {
        ...
        // If timeout, treat it like a "not found" error and retry
        if (__DEV__) {
          console.log('[AuthContext] Initial fetch timed out, retrying...');
        }
        error = { code: 'TIMEOUT', message: timeoutError.message } as any;
      }

      if (error) {
        // If profile doesn't exist (PGRST116 is "not found" error) OR timed out
        if (error.code === 'PGRST116' || error.code === 'TIMEOUT') {
          if (__DEV__) {
            console.log('[AuthContext] Profile not found or timed out, waiting for database trigger...');
          }

          // Wait a bit for the database trigger to create the profile
          await new Promise(resolve => setTimeout(resolve, 2000));

          // Try fetching again (with longer timeout)
          const retryFetchPromise = supabase
            .from('profiles')
            .select('*')
            .eq('id', userId)
            .single();

          const retryTimeoutPromise: Promise<never> = new Promise((_, reject) =>
            setTimeout(() => reject(new Error('Profile fetch timeout after 10s on retry')), 10000)
          );
          ...
          } catch (retryTimeoutError: any) {
            if (__DEV__) {
              console.error('[AuthContext] Retry also timed out');
            }
            retryError = { code: 'TIMEOUT', message: retryTimeoutError.message };
          }

          if (retryError) {
            const errorMsg = 'Profile not found after retry. Database trigger may not be working.';
            if (__DEV__) {
              console.error('[AuthContext]', errorMsg, retryError);
            }
            setProfileError(errorMsg);
            // Don't return - still keep the user logged in even without profile
            return;
          }
